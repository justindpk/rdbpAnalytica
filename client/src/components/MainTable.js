    ...
    <table>
      ...
      <tr>
        <th onClick={() => handleSort('rank', (a, b) => a.history[0].rank - b.history[0].rank)}
          ...
          Rank {'rank' === sortBy && !sortAscending ? '▲' : '▼'}</th>
        <th><img src="/img/duckIcon.svg" alt="duck" className='duckIcon'/></th>
        <th className={handleSortClassName('id')} onClick={() => handleSort('id', (a, b) => a.duck - b.duck)}>ID {'id' === sortBy && !sortAscending ? '▲' : '▼'}</th>
        <th className={handleSortClassName('version')} onClick={() => handleSort('version', (a, b) => a.history[0].version - b.history[0].version)}>
          Version {'version' === sortBy && !sortAscending ? '▲' : '▼'}</th>
        <th className={handleSortClassName('parties')} onClick={() => handleSort('parties', (a, b) => a.attributes[0].value - b.attributes[0].value)}>
          Parties {'parties' === sortBy && !sortAscending ? '▲' : '▼'}</th>
        <th className={handleSortClassName('rankChange')} onClick={() => handleSort('rankChange', (a, b) =>
          ...
          (b.history.length > 1 ? b.history[1].rank - b.history[0].rank : 0))}>Rank Change {'rankChange' === sortBy && !sortAscending ? '▲' : '▼'}
        </th>
        <th>Opensea</th>
      </tr>
      </thead>
      ...
      {databases['allDucks'].slice(0, amountToLoad).map((duck, index) => {
        return (
          <tr key={index}>
            ...
            <td className={handleSortClassName('parties')}>{duck.attributes[0].value} / {duck.attributes[0].max_value}</td>
            <td className={handleSortClassName('rankChange')}>{duck.history.length > 1 ? duck.history[1].rank - duck.history[0].rank : 0}</td>
            <td><a href={`https://opensea.io/assets/0x495f947276749ce646f68ac8c248420045cb7b5e/${duck.duck}`}
                   target="_blank" rel="noreferrer"><img src={"/img/opensea.svg"} alt="opensea" style={{width: '45px'}}/></a></td>
          </tr>
        )
      })}
